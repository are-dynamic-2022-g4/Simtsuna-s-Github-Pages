...
class Batiment: 
    ...
    def __init__(self,norme_Securité:int, hauteur:int, etat:float, coords:tuple ): #altitude
        """
        ...
        self.coords=coords
        self.etat=etat
        self.materiau=ceil(norme_Securité/2)
        
    def __repr__(self):
        return "% s % s % s % s % s" % (self.norme,self.hauteur, self.etat,self.materiau, self.coords)
        #return "Batiment resi:% s hauteur:% s etat:% s" % (self.resi, self.hauteur, self.etat) 

Maison1=Batiment(5,10,1,(2,5))
...
class Vagues:
    def __init__(self, magnitude, coords):
        if magnitude==-1:
            self.hauteur = 0.5
        elif magnitude==0:
            self.hauteur = 1
        ...
        self.magnitude=magnitude #(-1 à 4 sur l'échelle d'Imamura et Lida)
        self.coords=coords
    def __repr__(self):
        return "% s % s % s % s" % (self.magnitude, self.hauteur, self.pression, self.coords)
...
def gethauteur(norme):
    ...
    ref=[(800,800),(3,5),(5,8),(5,8),(8,28),(8,28),(28,50),(28,50),(50,120),(50,120),(120,150)]  #on pick 800 pour le bunker ultrasafe pck la hauteur n'importe pas (en vrai si mais ptt plus tard)
...
def genBoard(taille_ville:int ,taille_littoral:int ,taille_mer:int ,norme_moyenne:int ,nb_bats:int,nb_civils:int,infos_murs:list,profondeur_max):
    ...
    for i,j in coords_bat:
        ...
        board[i][j]=[Batiment(norme,hauteur,1,(i,j))]
        board[i][j][0].resi=lst_resi[board[i][j][0].materiau]
    board[0][0]=[Batiment(norme,hauteur,1,(i,j))]
    board[0][0][0].resi=lst_resi[board[i][j][0].materiau]
...
def hauteur_max(magnitude):
    ...
    *100 pour convertir en cm
    ...
    calc à partir de la profondeur !
    ...
    if magnitude==-1:
        ...
        hauteur = 2
        moy=randint(25-5,25+5)/10
    elif magnitude==2:
        hauteur = random.randint(4, 6)
        moy=randint(50-10,50+10)/10
    elif magnitude==3:
        hauteur = random.randint(10, 20)
        moy=randint(100-10,100+10)/10
    elif hauteur==4:
        hauteur = random.randint(20, 30)
        moy=randint(200-10,200+10)/10
    elif hauteur>4:
...
def Tsunami3(board,taille_ville,taille_littoral,taille_mer,magnitude):
    ...
    #for j in reversed(range(taille_ville+taille_littoral,taille_totale)):       #de droite à gauche
        for i in range(taille):
            Vague[i][j]=Vagues(magnitude,(i,j))
            Vague[i][j].hauteur=hauteur
            #Vagues(vitesse,coords,hauteur)
            #Vague(magnitude,coords:tuple)
...
def Tsunami4(board,taille_ville,taille_littoral,taille_mer,magnitude):
    ...
    taille_totale =len(board[0])
    taille=len(board)
    v=[[0 for x in range(taille_totale)] for y in range(taille)]
    hauteur,moy=hauteur_max(magnitude)
    print("hauteur: ",hauteur,moy)
    #for j in range(taille_ville+taille_littoral,taille_totale)):   #de gauche à droite
    for j in reversed(range(taille_ville+taille_littoral,taille_totale)):       #de droite à gauche
        for i in range(taille):
            v[i][j]=[Vagues(magnitude,(i,j))]
            v[i][j][0].hauteur=hauteur
        while hauteur-moy>0:
            hauteur-=moy     #ICI METTRE FORMULE CALCULE DE LA HAUTEUR
            #faire passer la hauteur en cm  ? (évite le -=0.3
        hauteur=randint(int(moy-moy*0.2),int(moy+moy*0.2))        #ICI METTRE FORMULE CALCULE DE LA HAUTEUR
        #FAIRE LES CALCULS DE LA VITESSE AUSSI  // ET ENERGIE ? 
        
    for i in range(taille):
        for j in range(taille_totale):
            if board[i][j]!=0:
                if isinstance(board[i][j],Vagues):
                    v[i][j][0]=board[i][j].hauteur
                else:
                    v[i][j]=board[i][j]
            else:
                v[i][j]=0
...
def cout(objet):
    ...
    if objet!=0:
        if isinstance(objet,Batiment):
            ...
            cout_construction=prix_resi[int(norme)]*1.2*etage*100

        else: #si c un mur du littoral
            norme=objet[0]
            hauteur=objet[1] #à vérifier 
            cout_construction=prix_resi[int(norme/10)]*hauteur*50*1.2
    return cout_construction           
    #prix d'une case vide en dégât ? 
    

# print(cout(env1[0][0][0]))
# print(cout(Batiment(3,15,1,(3,3))))
...
def budget(board,taille_ville,taille_littoral):
    ...
    print("Budget total du littoral: ",budget_littoral,"€ pour ",nb_mur," mur.")
...
lst_materiaux = ["détruit", "brique", "bois", "béton", "roche", "béton fibré"] 
...
def show_mat(ville, coords):
    ...
    if isinstance(ville[i][j][0], Batiment):
        ...
        print(lst_materiaux[ceil(ville[i][j][0].norme/2)])
        print(" ")
        return
    else:
